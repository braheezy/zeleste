...
- `src/runtime/background.zig`: BG0 room map streaming, BG1 parallax streaming,
  ...
  helpers.
- `src/runtime/bird_npc.zig`: reusable data-driven bird NPC/tutorial prompt
  actor. It owns per-room bird data loading, the prologue bird state machine,
  hint tile/palette streaming, OBJ drawing/hiding, cache invalidation, and the
  bridge-ending dash-prompt fly-in API.
- `src/runtime/camera.zig`: camera value type and basic player-follow camera
  calculations.
- `src/runtime/chapter_flow.zig`: current chapter/session flow owner. It owns
  gameplay room load/display hiding helpers, bridge-ending hold and dash unlock
  state, prologue end-level transition timing, dummy overworld handoff, and the
  chapter 1 city entry path from the placeholder overworld.
- `src/runtime/chimney_smoke.zig`: reusable procedural 16x16 OBJ smoke effect.
  It owns the generated smoke tiles, palette color, counter/update cadence, OBJ
  drawing, and hide/reset helpers; callers provide active state, object base,
  and origin.
...
- `src/runtime/cutscene_dialogue.zig`: reusable cutscene dialogue box renderer.
  ...
  placement.
- `src/runtime/cutscene_laugh_text.zig`: reusable floating cutscene text VFX
  extracted from Granny's laugh text. It owns the `HAHA` tile upload/cache,
  fixed object budget, particle state, update cadence, camera-follow retarget,
  draw/hide behavior, and exposes simple start/update/draw/stop hooks.
...
- `src/runtime/foreground_stamps.zig`: reusable foreground decoration and
  ...
  count used by room wires when sharing spare object slots.
- `src/runtime/funny_cars.zig`: reusable one-way platform actor extracted from
  generic stamp data. It owns car state, graphics/palette loading, bounce
  update, footstep/floor probes, one-way top queries, and OBJ drawing.
- `src/runtime/gameplay_scene.zig`: gameplay scene draw-order and scene-effect
  ...
  and cache invalidation, wind/snow scene hooks, chimney-smoke scene hooks,
  player object hiding, and object-slot constants shared by room systems.
- `src/runtime/granny_npc.zig`: prologue Granny NPC sprite renderer. It owns
  Granny animation frame caches, palette loading, OBJ draw/hide behavior, and
  fixed sprite offsets while `src/runtime/prologue_granny_cutscene.zig` chooses
...
- `src/runtime/room_systems.zig`: room lifecycle/update facade. It owns
  room-system loading, transient effect clearing, cutscene updates, dynamic
  hazard updates and snow side effects, room actor/effect updates, static room
  hazard checks, and the shared foreground animation counter.
...
- `src/runtime/room_wires.zig`: reusable room wire OBJ fallback system for rooms
  where wires cannot be stamped directly into BG tiles. It loads per-room wire
  chunks, uploads room-specific wire tiles, and borrows unused falling-block and
  behind-stamp object slots.
- `src/runtime/tiny_birds.zig`: reusable fixed-size tiny bird flock actor. It
  owns flock loading, persistent flown state, trigger/update behavior,
...
- `src/runtime/prologue_bridge.zig`: prologue bridge/end-platform room system.
  It owns bridge asset loading, chunk collapse state, bridge OBJ drawing,
...
- `src/runtime/prologue_granny_cutscene.zig`: prologue Granny cutscene script.
  It owns trigger/progress state, player cutscene locking, dialogue page/reveal
  state, dialogue overlay calls, laugh-text lifetime, Granny NPC pose/facing
  selection, room palette mood, cutscene camera shake, and death/room-transition
  cleanup hooks.
...
1. poll input;
2. let `chapter_flow` consume the frame if an end-level/overworld transition is
   active;
...
- `loadRoomSystems` loads room-owned systems after the BG/palette data is
  loaded. Current systems include falling blocks, foreground stamps, funny
  cars, prologue bridge state, bird NPCs, tiny birds, room wires, and parallax.
- `updateRoomCutscenes` and `updateRoomCutsceneEffects` are the cutscene entry
  points. `src/runtime/prologue_granny_cutscene.zig` is the first authored
  script behind these hooks; additional chapter cutscenes should plug in here
  rather than adding direct branches in the main loop.
...
- `updateRoomActors` is for non-player room actors and NPCs, including the
  reusable bird NPC, Granny cutscene/NPC integration, funny car platforms, tiny
  bird flocks, and bridge actors.
...
- `src/runtime/chapter_flow.zig` owns the current prologue-to-overworld-to-city
  session handoff. Future chapter end screens or overworld nodes should extend
  that boundary instead of adding phase globals to the frame loop.
...
- `src/runtime/funny_cars.zig` plugs into the same one-way top query path so
  reusable actor-backed platforms can support movement without entering the
  static tile collision module.
...
- `src/runtime/room_wires.zig` draws OBJ wires only when static BG stamping is
  not possible. It borrows unused falling-block and behind-stamp object slots.
