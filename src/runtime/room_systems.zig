...
pub fn load(room_index: usize, reset_cutscenes: bool) void {
    ...
    chapter_systems.loadBeforeObjectSprites(room_index, .{
        .funny_car_first_object = gameplay_scene.funny_car_first_object,
    });
...
pub fn updatePlayerEffects(player: *Player) void {
    hair.update(player, chapter_systems.endingHairOverrideActive());
    dust.update();
    dash_effects.update();
}
